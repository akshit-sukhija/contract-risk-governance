...
This module generates a human-readable explanation from an already computed
trace and final decision. It does not participate in decision logic.
"""
...
def generate_ai_explanation(trace: list, final_decision: str) -> str:
    """Generate a plain-language explanation from trace and final decision.

    On any API/token/network/parsing issue, returns a deterministic fallback
    explanation without raising exceptions.
    """
    ...
    try:
        ...
        data = response.json()
        explanation = _extract_generated_text(data)
        if not explanation:
            return fallback
...
def _build_prompt(trace: List[Any], final_decision: str) -> str:
    ...
    for index, entry in enumerate(trace[:MAX_TRACE_ITEMS], start=1):
        if not isinstance(entry, dict):
            trace_lines.append(f"{index}. {str(entry)}")
            continue
